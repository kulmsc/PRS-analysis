...
boostEval <- function(authorComp,bestFile,compStat,trainDf,testDf,allSupportDecode,allDiseaseDecode,
                      trainROC,trainAUC,testROC,testAUC,maxBest, removeSimilar,decoder,currentTrait){
...
parallelBT <- function(pullCol,trainDf,supportTrain,compStat,roc){
  ...
  completeLogistic <- glm(response ~ .,data=evalDf,family="binomial")
  ...
  if(compStat=="OR"){
    ...
    g=pROC::roc(response ~ pred,data=evalDf)
...
suppTrain=data.frame(matrix(0,nrow=nrow(trainDf),ncol=nrow(bestTracker)))
suppTest=data.frame(matrix(0,nrow=nrow(testDf),ncol=nrow(bestTracker)))
...
for(suppFile in unique(bestTracker$bestFile)){
  ...
  suppTrain[,bestTracker$bestFile == suppFile] <- as.data.frame(supportTrain[,which(colnames(supportTrain) %in% rownames(bestTracker)[bestTracker$bestFile==suppFile])])
  suppTest[,bestTracker$bestFile == suppFile] <- as.data.frame(supportTest[,which(colnames(supportTest) %in% rownames(bestTracker)[bestTracker$bestFile==suppFile])])
}
...
supportDecode <- supportDecode[supportDecode$author %in% rownames(bestTracker),]
...
makePlotsColor <- function(allRes,trainScores,translateLabels,totalTraits,numberColor){
  ...
  thePlot=ggplot(aucImpsDf,aes(traits,aucImps))+geom_point(aes(color=traits),size=4)+geom_errorbar(aes(ymin=lo,ymax=hi),width=0.2)+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10),plot.margin = unit(c(0.5,0.5,0.5,2),"cm"))+
    labs(x="Scores",y="AUC Improvement",title="More Stats: AUC Improvement",caption=paste("Models include",currentTrait,"score"))+
  ...
  thePlot=ggplot(pvalsDf,aes(traits,pvals,fill=traits))+geom_col()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10),plot.margin = unit(c(0.5,0.5,0.5,2),"cm"))+
    labs(x="Scores",y="logistic regression PVal",title=paste("More Stats: Logistic Regression PVals"),
  ...
  thePlot=ggplot(orPvalDf,aes(pvals,ors,label=word))+
    ...
    labs(x="PValue",y="Odds Ratio",title="More Stats: Effect and Certainty",caption=paste("Models include",currentTrait,"score"))+
  ...
  orsDf[,2]=factor(orsDf[,2],levels=c(0.5,0.2,0.1,0.05,0.01,0.005))
  thePlot=ggplot(orsDf,aes(variable,value,color=traits))+geom_line(aes(group=traits))+
    labs(x="Percentile Cut-Off",y="Odds Ratio",title="More Stats: Odds ratios",caption=paste("Models include",currentTrait,"score"))+
  ...
  thePlot=ggplot(prevsDf,aes(variable,value,color=traits))+geom_smooth(se=F)+
    labs(x="Score Percentile",y="Trait Prevalence",title="More Stats: Disease Prevalences",
         caption=paste("Models include",currentTrait,"score"))+
    scale_color_manual(values=specColors)+guides(color=FALSE)
  plot(thePlot)
}
...
print(dim(forPlot))
print("912")
print(sum(coding6$coding %in% forPlot$traits))
print(sum(forPlot$traits %in% coding6$coding))
...
thePlot=ggplot(forPlot,aes(traits,scores))+geom_col(aes(fill=traits))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1),plot.margin = unit(c(0.5,0.5,0.5,2),"cm"))+
  labs(x="Added Trait",y="AUC Improvement",title="Regularized LogReg - Traits",subtitle=paste("For Trait",currentTrait))+
  guides(fill=FALSE)
plot(thePlot)
...
return(list(sendBackBestTracker,suppTrain,suppTest,forPlot,finalAuc,bestScoreAUC,allResReturn,scoreStatsReturn,suppTrain,totalPhenoTrain,splitVnosplitAucs,stumpVregAucs))
}
