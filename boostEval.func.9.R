...
boostEval <- function(authorComp,bestFile,compStat,trainDf,testDf,allSupportDecode,allDiseaseDecode,
                      trainROC,trainAUC,testROC,testAUC,maxBest, removeSimilar){
...
if(removeSimilar){
  checkDecoder=allDiseaseDecode[allDiseaseDecode$author %in% commonNames,]
  checkCodes=str_split(checkDecoder$code,",",simplify = T)
  currentCodes=str_split(decoder[3],",")[[1]]
  for(code in currentCodes){
    codeStatus = apply(checkCodes,1,function(x) code %in% x)
    badNames=c(badNames,checkDecoder$author[codeStatus])
  }
}
...
if(removeSimilar){
  ...
  badTraits=nameNumberDecoder[nameNumberDecoder[,2] %in% currentCodes,1]
  if(length(badTraits) > 0){
    totalPhenoTrain=totalPhenoTrain[,-which(colnames(totalPhenoTrain) %in% badTraits)]
    totalPhenoTest=totalPhenoTest[,-which(colnames(totalPhenoTest) %in% badTraits)]
  }
}
