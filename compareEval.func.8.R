...
compareEval<-function(authorComp,finalCrit,plotAll,decoder,key,pheno,diseaseTrainList,cp){
...
for(f in authorFiles){
  ...
  print(which(colnames(allScores)==f))
  print(which(colnames(goodDiseaseFile)==authorComp))
...
#fix the pheno file
allScores=allScores[!is.na(pheno[,6]),,drop=F]
...
thePlot=ggplot(totalResMat,aes(parameter,toComp))+geom_point(aes(color=methods),size=4)+
  ...
  guides(color=FALSE)+theme(axis.text.x = element_text(angle = 45, hjust = 1, size=10), plot.margin = unit(c(0.5,0.5,0.5,2),"cm"))+
